...
option.list <- list(
  ...
  make_option("--outfile", type="character", default="", help="Output file. Default: stdout"),
...
if (any(duplicated(samplesheet$SampleID))) {
	stop("Found duplicate SampleIDs:\n", paste0(with(samplesheet, SampleID[duplicated(SampleID)]), collapse="\n"))
}

i5 <- read.delim(opt$i5) %>% select(Index2,Index2Sequence)
i7 <- read.delim(opt$i7) %>% select(Index1,Index1Sequence)
